---
title: PRoR Is Not Seasonal Storage
type: claim
created: 2026-04-14
updated: 2026-04-14
claim_id: C-009
confidence: high
status: stable
providers: [hermes, claude, codex, gemini]
sources: [wecs-energy-synopsis-2024, wb-nepal-power-sector-reform-2022, data-fleet-composition]
tags: [pror, peaking, storage, dispatch]
---

## Claim

**Peaking run-of-river (PRoR)** is **not** the same as **seasonal storage**. Treating PRoR as a substitute for multi-month reservoir regulation is a category error with strategic implications.

## Evidence

- **PRoR function:** Provider consensus describes PRoR as helping with **daily peaking** windows on the order of roughly **2–6 hours**—valuable for intraday shaping, not for carrying monsoon energy into dry-season months.
- **Nepal's binding mismatch is multi-month:** Wet-season energy surplus vs dry-season scarcity is a **seasonal** problem; daily pondage does not re-time bulk energy across monsoon→winter unless paired with very large effective storage volumes or exogenous balancing (trade, demand migration, etc.).
- **Hermes:** Calls the PRoR-as-storage substitution "analytically sloppy and strategically dangerous"—because it invites under-investment in true seasonal buffers and mis-prices firm capacity.
- **Claude / Codex / Gemini:** Align on separating **intraday flexibility** from **inter-seasonal energy inventory**.

## Confidence rationale

**High** because the distinction is definitional in hydropower engineering (pondage volume and hours of storage vs seasonal carryover). Debates remain over how much daily peaking helps *operationally*, not over whether PRoR equals seasonal storage.

## Unresolved issues

- Some PRoR designs couple with **upstream regulation** or **cascades** in ways that blur public labeling—taxonomy needs care in [[data-fleet-composition]].
- Optimal **mixed portfolio** (PRoR + storage + trade) is an open planning question; this claim only blocks a false equivalence.

## Related

- [[run-of-river-hydropower]]
- [[storage-deficit]]
- [[firm-power]]
