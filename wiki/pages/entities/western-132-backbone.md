---
title: Western Nepal 132 kV Backbone
type: entity
category: transmission-corridor
created: 2026-04-25
updated: 2026-04-25
sources: [nepal-transmission-landscape-2025, nea-annual-report-fy2024-25]
tags: [transmission, 132kv, western-nepal, lumbini, sudurpaschim]
generator: manual
---

# Western Nepal 132 kV Backbone

The operational 132 kV Terai-side grid chain that carries the map west of Butwal: Butwal-Shivapur-Lamahi-Kohalpur, Kohalpur-Bhurigaun-Lamki, and Lamki-Pahalwanpur-Attariya-Mahendranagar.

## Key Facts

| Parameter | Value |
|-----------|-------|
| Voltage | 132 kV |
| Status | Operational |
| Length basis | 404.89 route-km / 809.78 circuit-km |
| Source geometry | RPGCL 2021 official geospatial vector map |
| Status/length basis | NEA FY 2024/25 existing high-voltage line inventory |
| Corridor ID | `western_132_backbone` |

## Map Interpretation

This corridor is now in the major transmission network because otherwise western Nepal looks emptier than it is. It is not a 400 kV export spine or a 220 kV bulk-transfer corridor; it is the operational 132 kV backbone that keeps Lumbini, Banke, Bardiya, Kailali, and Kanchanpur connected across the west.

Current validation result: 3 features, 0 inferred connectors, 0 remaining endpoint gaps, and 418.149 km traced length against a 404.89 route-km basis. Public decision: default-visible, caveated.

## Caveats

The route is source-backed but national-scale. It comes from RPGCL vector linework and NEA inventory control, not a tower-by-tower alignment sheet. It should be read as the western grid-reach backbone, with lighter visual weight than 220/400 kV lines.

## See also

- [[data-layer-transmission-connected-traced-network]]
- [[chameliya-attariya-132]]
- [[kohalpur-surkhet-dailekh-132]]
- [[dana-kushma-butwal-corridor]]
- [[gorakhpur-butwal-interconnection]]
