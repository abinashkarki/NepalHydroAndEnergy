---
title: Chera-1
type: entity
created: 2026-05-06
updated: 2026-05-06
generator: specs-refresh
page_quality: analysis
sources: [jica-ipsdp-main-report-vol2, moewri-ipsdp-exec-summary-2025]
tags: [project, storage, karnali]
images: []
---

# Chera-1

Chera-1 is a 149 MW JICA promising-storage candidate in the Karnali basin. It is
pinned in [[data-layer-storage-shortlist]].

<!-- generated:specs:start -->

## Specifications

| Parameter | Value |
|-----------|-------|
| Capacity | 149 MW |
| Category | jica promising storage |
| Basin | Karnali ([[karnali-basin]]) |
| Annual energy | 563.2 GWh |
| Dry-season energy | 120.6 GWh |
| Dry share | 21.4% |
| Total storage | 254.9 MCM |
| Effective storage | 141.1 MCM |

<!-- generated:specs:end -->

<!-- generated:sources:start -->

## Sources

- [[jica-ipsdp-main-report-vol2|JICA IPSDP Final Report Annexes (Vol. 2)]]
- [[moewri-ipsdp-exec-summary-2025|MoEWRI IPSDP Executive Summary]]

<!-- generated:sources:end -->

## Why It Matters

Chera-1 offers **563.2 GWh** of annual energy with **120.6 GWh** of dry-season output
(21.4% dry share) — a meaningful contribution to Karnali-basin firming, though its
storage volumes are more modest than the larger shortlist candidates on the Karnali
main stem.

## Status And Timing

Chera-1 should be read as a planning / promising-storage candidate surfacing from the
JICA/NEA storage master plan. The IPSDP milestone ladder may place it in a later
windows (2031–2040). There is no confirmed construction start in the current wiki
evidence set.

## Map Interpretation

The map marker is a river-aligned project reference. Use it for basin-level and
strategic comparison, not for final dam-axis coordinates.

## Registry note

- JICA/NEA storage master plan volume 2

## See also

- [[karnali-basin]]
- [[storage-deficit]]
- [[firm-power]]
- [[data-layer-storage-shortlist]]
- [[data-ipsdp-milestone-ladder-2022-2040]]
